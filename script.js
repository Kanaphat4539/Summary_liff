...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbwfMqjDFX04L_Yg8lc42rC2-ib6j6OTPUvEFeE0fHhB2wGpy7CbL9Pf2mgeWwTYSavXCg/exec';
...
function renderLogsList(logs) {
    ...
    listContainer.innerHTML = logs.map(log => {
        const hasOver = String(log.System_Log || '').toLowerCase().includes('over');
        const hasError = String(log.System_Log || '').toLowerCase().includes('error');
        
        let footerClass = '';
        if (hasOver) footerClass = 'has-over';
        else if (hasError) footerClass = 'has-error';
        ...
        if (log.Timestamp) {
            try {
                ...
                if (!isNaN(dateObj.getTime())) {
                    ...
                    displayTime = log.Timestamp; // fallback
                }
            } catch (e) {
        ...
        const sysParsed = parseSystemLog(log.System_Log);
        const sysText = log.System_Log ? `
            <div style="font-size: 11px; color: var(--c-text-muted); margin-bottom: 8px; background:#f1f5f9; padding: 6px 10px; border-radius:8px;">
                💡 ถุงใบที่ #${sysParsed.bagNo} • อาการ: ${sysParsed.errorType} • น้ำหนักจริง: ${sysParsed.weight || 'N/A'} kg
            </div>
        ` : '';
            ...
            <div class="log-card">
                <div class="log-card-header">
                    <div class="log-card-title">
                        <span class="machine-badge">${escHtml(log.Machine)}</span>
                        <span class="sp-badge">${escHtml(log.SP_No)}</span>
                    </div>
                    <span class="status-badge completed">${escHtml(log.Repair_Status || 'ซ่อมสำเร็จ')}</span>
                </div>
                <div class="log-card-grid">
                    <div class="log-grid-item">
                        <span class="log-grid-label">เคสไอดี</span>
                        <span class="log-grid-value">${escHtml(log.Case_ID || 'CASE-' + Math.floor(1000 + Math.random()*9000))}</span>
                    </div>
                    <div class="log-grid-item">
                    ...
                    <div class="log-grid-item">
                        ...
                        <span class="log-grid-value">${escHtml(log.Technician || 'ช่างซ่อมบำรุง')}</span>
                    </div>
                    <div class="log-grid-item">
                        <span class="log-grid-label">Downtime / Loss</span>
                        <span class="log-grid-value">
                            <span class="downtime">${log.Downtime_Min || 0} นาที</span> / 
                            <span class="loss">${formatNumberWithCommas(log.Loss_Coss || 0)} ฿</span>
                        </span>
                    </div>
                </div>
                ${sysText}
                <div class="log-card-footer ${footerClass}">
                    <strong>การดำเนินการ:</strong> ${escHtml(log.รายละเอียดงาน || 'เคลียร์เศษหินปูนและตั้งค่าระบบปกติ')}
                </div>
            </div>
        `;
    }).join('');
...
function filterCasesLog() {
    const val = document.getElementById('search-log-input').value.toLowerCase().trim();
    if (!val) {
        renderLogsList(loadedLogs);
        return;
    }
    ...
    const filtered = loadedLogs.filter(log => {
        return (log.Machine || '').toLowerCase().includes(val) ||
               (log.SP_No || '').toLowerCase().includes(val) ||
               (log.Technician || '').toLowerCase().includes(val) ||
               (log.Case_ID || '').toLowerCase().includes(val) ||
               (log.รายละเอียดงาน || '').toLowerCase().includes(val) ||
               (log.System_Log || '').toLowerCase().includes(val);
    });
...
async function openCaseDetails(machine, spNo) {
    ...
    try {
        ...
        if (data.success && data.jobs) {
            ...
            // Find active caution job
            job = loadedJobs.find(j => j.Machine === machine && j.SP_No === spNo);
        }
    } catch (err) {
